...
# --- 設定應用程式版本 ---
APP_VERSION = "v14.6 終極防護版 (防卡死+資料防呆)"
...
def get_stock_info_cached(ticker_symbol):
    ...
    t.start()
    t.join(timeout=2.0) # ★ 只等 2 秒，超時就放棄

    if 'data' in result:
        INFO_CACHE[ticker_symbol] = result['data']
        return result['data']
    else:
        return {'eps': 'N/A', 'pe': 'N/A'} # 超時回傳 N/A
