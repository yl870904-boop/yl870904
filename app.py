...
def calculate_score(df_cand, weights):
    ...
    df_cand['total_score'] = (score_trend * weights['trend'] + df_cand['score_momentum'] * weights['momentum'] + df_cand['score_risk'] * weights['risk'])
    ...
    with plot_lock:
        try:
            ...
            try:
                ...
                ax1.plot(df.index, df['Close'], color='black', alpha=0.6)
                if len(df) >= 20: ax1.plot(df.index, df['MA20'], color='orange', linestyle='--')
                if len(df) >= 60: ax1.plot(df.index, df['MA60'], color='blue')
                
                title_font = my_font if my_font else None
                try: ax1.set_title(f"{stock_name} 分析", fontproperties=title_font, fontsize=16)
                except: ax1.set_title(f"{target}", fontsize=16)
                ax1.grid(True, alpha=0.3)

                ax2 = fig.add_subplot(3, 1, 2)
                cols = ['red' if c >= o else 'green' for c, o in zip(df['Close'], df['Open'])]
                ax2.bar(df.index, df['Volume'], color=cols, alpha=0.6)
                ...
                fig.savefig(filepath, bbox_inches='tight', dpi=80) # 降低 DPI 減少記憶體
                ...
                result_file = None # 確保回傳 None
                
        except Exception as e:
            return None, f"數據分析失敗: {str(e)}\n{traceback.format_exc()}"
        finally:
            gc.collect()
        ...
        for stock in watch_list:
            try:
                if isinstance(data.columns, pd.MultiIndex):
                    try: c = data['Close'][stock]; v = data['Volume'][stock]; h = data['High'][stock]; l = data['Low'][stock]
                    except: continue
                else:
